...
const FRONTEND_URL = process.env.FRONTEND_URL || 'https://biotech-department-pob4.vercel.app';
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  body { font-family: Arial, sans-serif; width: 80mm; margin: 0 auto; padding: 8mm 5mm; color: #1f2937; background: white; font-size: 11px; }
  .logos { display: flex; justify-content: center; align-items: center; gap: 10px; margin-bottom: 8px; }
  ...
  .receipt-title { text-align: center; font-size: 11px; font-weight: bold; text-transform: uppercase; letter-spacing: 1px; margin: 8px 0 4px; }
  .receipt-number { text-align: center; background: #f0fdf4; border: 1px solid #15803d; padding: 4px 8px; border-radius: 4px; font-family: monospace; font-size: 12px; font-weight: bold; color: #15803d; margin-bottom: 8px; }
  ...
  .amount-box { background: #f0fdf4; border: 1px solid #15803d; border-radius: 4px; padding: 6px 8px; margin: 8px 0; display: flex; justify-content: space-between; align-items: center; }
  .amount-label { font-size: 11px; font-weight: bold; }
  .amount-value { font-size: 14px; font-weight: bold; color: #15803d; }
  .stamp { text-align: center; border: 2px solid #15803d; border-radius: 50%; width: 50px; height: 50px; display: flex; align-items: center; justify-content: center; margin: 8px auto; color: #15803d; font-weight: bold; font-size: 10px; line-height: 1.2; }
  .footer { text-align: center; margin-top: 8px; padding-top: 6px; border-top: 1px dashed #d1d5db; color: #9ca3af; font-size: 9px; line-height: 1.5; }
  @media print { body { width: 80mm; margin: 0; padding: 5mm; } @page { size: 80mm auto; margin: 0; } }
</style>
</head>
<body>
  <div class="header">
    <div class="logos">
      ${fulLogo ? `<img src="${fulLogo}" class="logo-img" alt="FUL">` : ''}
      ${deptLogo ? `<img src="${deptLogo}" class="logo-img" alt="Dept">` : ''}
    </div>
  ...
  <div class="row"><span class="label">Matric No</span><span class="value">${r.matric_number}</span></div>
  <div class="row"><span class="label">Level</span><span class="value">${r.level}</span></div>
  <div class="row"><span class="label">Session</span><span class="value">${r.academic_session}</span></div>
  <div class="row"><span class="label">Date</span><span class="value">${new Date(r.issued_at).toLocaleDateString('en-NG', { day: 'numeric', month: 'short', year: 'numeric' })}</span></div>
  <div class="row"><span class="label">Ref</span><span class="value" style="font-size:8px">${r.payment_reference}</span></div>
  <hr class="divider">
