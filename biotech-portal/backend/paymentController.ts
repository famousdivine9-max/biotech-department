...
export const paystackWebhook = async (req: any, res: Response): Promise<void> => {
  try {
    ...
    if (event === 'charge.success') {
      await pool.query("UPDATE payments SET payment_status = 'successful', payment_date = NOW() WHERE paystack_reference = $1 AND payment_status = 'pending'", [data.reference]);
    }
    res.sendStatus(200);
  } catch (error) {
    res.sendStatus(500);
...
export const downloadReceipt = async (req: any, res: Response): Promise<void> => {
  try {
    ...
    if (result.rows.length === 0) { res.status(404).json({ success: false, message: 'Receipt not found.' }); return; }
